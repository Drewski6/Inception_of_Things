...
until ssh -o StrictHostKeyChecking=no -o ConnectTimeout=2 vagrant@"$SERVER_IP" "sudo test -s $TOKEN_DST"; do
  echo "waiting for server token..."
  sleep 2
done

K3S_TOKEN="$(ssh -o StrictHostKeyChecking=no vagrant@"$SERVER_IP" "sudo cat $TOKEN_DST")"
