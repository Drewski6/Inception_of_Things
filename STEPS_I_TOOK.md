...
- Things should set up automatically from there.

### Additional tools

- In addition I created 2 connect and 2 stop scripts to help in the `/scripts` folder. These scripts can be used to launch the WebGUI for ArgoCD and stop it. Likewise, it can open the port to the app's server port and stop the port forward.
- The connect scripts work by using the `kubectl port-forward` command, sending stdout to a log file, and sending stderr to stdout (also the logfile), then making the command a job.
- We then save the pid created by the previous command in a pid file where we can kill the process later if we want to stop it.
...
- The stop scripts simply read the file we created in the connect script to get the right pid, and kills it.
...
- From here we just need to sync in our ArgoCD WebGUI which is easy enough to do.

- Then the subject asks that you be able to change the tag. Just go into `deployment.yaml` and change the version from `v1` to `v2` and sync. The process after that should be automatic.

- You can test that everything works and that the tag is correct using a basic curl `curl localhost:8888`
